...
def peptideCollapse (data, cutoff = 0, collapse_level = 'PG'):
    """
    ...
    def fun4(entry1, entry2):
        ...
        return(res)
    ##################
    print ('Start of dataset preprocessing:'+ ' ' + 'dataset consists of' + ' ' + str(data.shape[0]) + ' ' + 'ID rows in' + ' ' + str(len(list(data['R.FileName'].unique()))) + ' ' + 'samples')
    ...
    data['PTM_pep_pos'] = data['PEP.PeptidePosition'].astype(str).apply(lambda row:list(filter(None, row.split(';')))[0])
    ...
    data['PG.ProteinGroups'] = data['PG.ProteinGroups'].apply(lambda row: row.replace('_', '-'))
    data['PTM_Genprot'] = data['PTM_Genprot'].apply(lambda row: row.replace('_', '-'))
    data['PTM_Collapse_key'] = data.apply(lambda x: fun3(x['PG.ProteinGroups'],x['PTM_Genprot'], x['PTM_0_aa'],x['PTM_0_pos_val'],
                                                  x['PTM_pep_pos'],x['PTM_mult123']), axis = 1)
    num3 = num2 - data.shape[0]
    print('Preparation for collapsing done;'+' '+str(num3)+ ' '+'phosphopeptides filtered due to insufficient information') 
    #################
    cols = []
    for c in fine_names:
    ...
    if collapse_level == 'P':
        ...
        data['PTM_Collapse_key'] = data['Protein_group'] + '~' + data['Gene_group'] + '_' + data['Site'] + '_' + data['Mult']
        data['Protein_Collapse_key'] = data['Protein_name'] + '~' + data['Gene_group'] + '_' + data['Site'] + '_' + data['Mult']
    ...
    if collapse_level == 'PG':
        ...
        data['Gene_name'] = data['Gene_name'].str.split(';')
        data['Gene_group'] = data['Gene_name'].apply(lambda row: list(row)[0])
        data = data.drop(['Gene_name','Protein_name','PTM', 'PTM_Collapse_key', 'PG.Genes', 'PG.ProteinGroups'], axis = 1)
        data['PTM_Collapse_key'] = data['Protein_group'] + '~' + data['Gene_group'] + '_' + data['Site'] + '_' + data['Mult']
        data = data.drop(['Site', 'Mult'], axis = 1)
        print('Collapse done succesfully;' + ' ' +str(data.shape[0]) + ' ' + 'unique phosphosite profiles')
        et = time.time()
...
def prepare_phosphositeplus_dataset(dataset, organism1, organism2):
    data_copy = dataset.copy()
    data_copy = data_copy[data_copy['MOD_RSD'].str.contains('-p')]
    data_copy['MOD_RSD'] = data_copy['MOD_RSD'].apply(lambda row: row.split('-')[0])
    data_copy['Protein_key'] = data_copy['ACC_ID'] + '~' + data_copy['GENE'] + '_' + data_copy['MOD_RSD']
    organism1_df = data_copy[data_copy['ORGANISM'] == organism1]
    organism2_df = data_copy[data_copy['ORGANISM'] == organism2]
    return(organism1_df, organism2_df)
    

def translate_phosphosites (dataset, phosphosites_plus_dataset, base_organism = 'mouse', translation_organism = 'human', drop_cols = ['sample', 'subject'], index_col = 'group', id_column = 'Protein_Collapse_key'):
    mouse_df, human_df = prepare_phosphositeplus_dataset(dataset= phosphosites_plus_dataset, organism1= base_organism, organism2=translation_organism)
    data_copy = dataset.copy()
    data_sample = data_copy['sample'].to_list()
    data_subject = data_copy['subject'].to_list()
    data_copy = data_copy.drop(drop_cols, axis = 1).set_index(index_col).T
    data_copy.index.name = id_column
    data_copy = data_copy.reset_index()
    data_copy[['tmp1', 'tmp2', 'tmp3']] = data_copy[id_column].str.split('_',expand = True)
    data_copy[id_column] = data_copy['tmp1'] + '_' + data_copy['tmp2']
    data_copy = data_copy.drop(['tmp1', 'tmp2', 'tmp3'], axis = 1)
    ###
    def fun(row):
        if row in mouse_df['Protein_key'].to_list():
            return(str(mouse_df[mouse_df['Protein_key'] == row]['SITE_GRP_ID'].unique()[0]))
        else:
            return(np.nan)
    ###
    data_copy['Index'] = data_copy[id_column].apply(lambda row: fun(row))
    data_copy = data_copy.dropna()
    human_df['SITE_GRP_ID'] = human_df['SITE_GRP_ID'].apply(lambda row: str(row))
    human_df = human_df[['Protein_key', 'SITE_GRP_ID']]
    human_df.columns = ['Protein_key', 'Index']
    data_copy = data_copy.merge(human_df, on = 'Index', how = 'inner')
    data_copy[id_column] = data_copy['Protein_key']
    data_copy = data_copy.drop(['Protein_key', 'Index'], axis = 1)
    data_copy['Protein_Collapse_key'] = data_copy['Protein_Collapse_key'] + '_' + 'MNA'
    data_copy = data_copy.set_index(id_column)
    data_copy.index.name = ''
    data_copy = data_copy.T
    data_copy.index.name = 'group'
    data_copy = data_copy.reset_index()
    data_copy = data_copy.reset_index().drop('index', axis = 1)
    data_copy['sample'] = data_sample
    data_copy['subject'] = data_subject
    return(data_copy)
...
def set_condition (data, condition_list, collapse_level = 'PG'):
    ...
    if collapse_level == 'P':
        index_cols = ['UPD_seq', 'PTM_localization', 'Protein_name', 'Protein_group', 'Gene_group',
                      'PTM_Collapse_key', 'Protein_Collapse_key']
    if collapse_level == 'PG':
        index_cols = ['UPD_seq', 'PTM_localization', 'Protein_group', 'Gene_group', 'PTM_Collapse_key']
...
def integrate_funscores(data, funscore_data, index_cols = ['group', 'sample', 'subject'], threshold = 0.5):
    """
    Filters dataset for highly functional phosphosites (Ochoa et al, 2020)
    
    :param pd.DataFrame data: collapsed dataframe with condition setup
    :param pd.DataFrame data: functional scores data obtained from R script that can found in Ochoa et al, 2020 
    :param list index_cols: columns used for indexing, default [group, sample, subject]
    :param float threshold: threshold used for filtering of functional phosphosites, default 0.5
    :return pd.DataFrame data: filtered dataset

    Example::
        functional_dataframe = integrate_funscores(data = condition_dataframe, funscores_data, index_cols = ['group', 'sample', 'subject'], threshold = 0.5)
    """
    
    
    if data.shape[0] > data.shape[1]:
        print('Dataframe was not transformed properly. Please use peptideCollapse() and set_condition() first')
        return(data)
    else: print('Phosphosite dataframe is valid')
    if 'sites' in funscore_data.columns or 'probabilities' in funscore_data.columns:
        print('Funscores dataframe is valid')
    else:
        print ('Funscores dataframe does not have sufficient information')
        return(data)
    funscore_data = funscore_data[funscore_data['probabilities']>= threshold]
    data_copy = data
    data_copy = data_copy.set_index(index_cols)
    cols = np.array(data_copy.columns)
    tmp1 = 0
    tmp2 = 0
    t = 0
    tmp3 = []
    for col in cols:
        tmp1 = col.split('~')[0]
        tmp2 = col.split('_')[1][1:]
        t = tmp1 + '_' + tmp2
        tmp3.append(t)
    data_copy = data_copy.T
    data_copy['tmp_cols'] = tmp3
    data_copy = data_copy[data_copy['tmp_cols'].isin(funscore_data['sites'])]
    data_copy = data_copy.drop('tmp_cols', axis = 1).T.reset_index()
    return(data_copy)


def prepare_kinact_input(dataset, condition_dict, control_group, drop_cols = ['sample', 'subject'], index_col = 'group'):
    """
    Prepares dataset for kinact analysis.
    :param pd.DataFrame() dataset: input imputed dataset
    :param dict condition_dict: condition setup for the experiment with condition groups as key and sample unique IDs as values
    :param str control_group: condition group ID that will be used as a control group
    :param list drop_cols: columns that will be dropped during preprocess, default: ['sample', 'subject']
    :param str index_col: column that will be used as index
    :return pd.DataFrame() df_tmp: output dataset
    """
    df_tmp = dataset
    ### reset condition dictionary if it does not have control key as first row
    tmp_cond = pd.DataFrame(condition_dict).T
    tmp_cond1 = tmp_cond[tmp_cond.index == control_group].T
    tmp_cond2 = tmp_cond[tmp_cond.index != control_group].T
    tmp_cond3 = pd.concat([tmp_cond1, tmp_cond2], axis = 1).to_dict('list')
    ###
    df_tmp = df_tmp.drop(drop_cols, axis = 1).set_index(index_col).T
    df_tmp.index.name = 'PTM_Collapse_key'
    #calculating condition group median and fold change
    for key, value in tmp_cond3.items():
        if key == control_group:
            ctrl = key
            ctrl_exp = ctrl+'_median'
        exp = key+'_median'
        df_tmp[exp] = df_tmp[key].median(axis = 1)
        fc_exp = key+'_FC'
        df_tmp[fc_exp] = df_tmp[exp] - df_tmp[ctrl_exp]
    df_tmp = df_tmp.reset_index()
    #preparing phosphosite input format
    df_tmp[['New_key1', 'New_key2']] = df_tmp['PTM_Collapse_key'].str.split('~', expand = True)
    df_tmp[['New_key3', 'New_key4', 'New_key5']] = df_tmp['New_key2'].str.split('_', expand = True)
    df_tmp['KSEA_Collapse_key'] = df_tmp['New_key1']+'_'+df_tmp['New_key4']
    df_tmp = df_tmp.drop(['New_key1', 'New_key2','New_key3','New_key4','New_key5', 'PTM_Collapse_key'], axis = 1)
    df_tmp = df_tmp.set_index('KSEA_Collapse_key')
    return(df_tmp)



def execute_kinact(dataset, condition_dict, control_group, set_size = 5, pval = 0.05, organism_type = 'human'):
    """
    Runs kinact analysis.
    :param pd.DataFrame() dataset: input imputed dataset
    :param dict condition_dict: condition setup for the experiment with condition groups as key and sample unique IDs as values
    :param str control_group: condition group ID that will be used as a control group
    :param int set_size: minimal number of kinase substrates
    :param float pval: threshold p value to filter output dataset
    :param str organism_type: organism type to filter adjacency matrix, default: human
    :return pd.DataFrame() df_tmp: output dataset
    """
    #preprocess dataset to fit kinact requirements
    df_tmp = prepare_kinact_input(dataset = dataset, condition_dict = condition_dict, control_group = control_group)
    #get adjacency_matrix
    adjacency_matrix = kinact.get_kinase_targets(organism = organism_type)
    #start kinact
    result_scores = pd.DataFrame()
    result_p_values = pd.DataFrame()
    for key, values in condition_dict.items():
        if key != control_group:
            scores, p_values = kinact.ksea.ksea_mean(data_fc = df_tmp[key+'_FC'].dropna(),
                                                 interactions = adjacency_matrix,
                                                 mP = df_tmp[key+'_FC'].dropna().values.mean(),
                                                 delta = df_tmp[key+'_FC'].dropna().values.std(),
                                                 minimum_set_size = set_size)
            result_scores[key] = scores
            result_p_values[key] = p_values
    results_filtered = result_scores * np.where(result_p_values < pval, 1, np.NaN)
    return(results_filtered)

def generate_mfuzz_input(anova_dataset, background_dataset, drop_cols = ['sample', 'subject'], index_col = 'group'):
    """
    Generates input for cluster analysis.
    :param pd.DataFrame() anova_dataset: anova output dataset
    :param pd.DataFrame() background_dataset: imputed dataset with group intensities
    :param list drop_cols: columns that will be dropped during preprocess, default: ['sample', 'subject']
    :param str index_col: column that will be used as index, default: group
    """
    df_tmp = anova_dataset[anova_dataset['rejected'] == True]['identifier'].to_list()
    df_tmp1 = background_dataset.drop(drop_cols, axis = 1).groupby(index_col).median().T
    df_tmp1.index.name = 'PTM_Collapse_key'
    df_tmp1 = df_tmp1.reset_index()
    df_tmp1 = df_tmp1[df_tmp1['PTM_Collapse_key'].isin(df_tmp)]
    return(df_tmp1)

def run_mfuzz_clustering(dataset, nclusters = 3, fuzzifier = 2.0, max_iters = 100, drop_col = 'PTM_Collapse_key'):
    """
    Runs soft fuzzy c-means clustering with skfuzzy package.
    :param pd.DataFrame() dataset: output dataset from generate_mfuzz_input function
    :param int nclusters: number of clusters, default: 3
    :param float fuzzifier: fuzzifier m index, usually between 1.5 and 2.0, default: 2.0
    :param int max_iters: maximal number of iterations to find cluster centers, default: 100
    :param str drop_col: default: 'PTM_Collapse_key'
    """
    data_copy = dataset.copy()
    data_index = data_copy[drop_col].to_list()
    data_copy = data_copy.drop(drop_col, axis = 1).reset_index().drop('index', axis = 1)
    data_cols = data_copy.columns
    np.random.seed(42)
    data_copy = stats.zscore(data_copy, axis = 1)
    data_copy = data_copy.to_numpy()
    cntr, u, u0, d, jm, p, fpc = fuzz.cluster.cmeans(data_copy.T, nclusters, fuzzifier, error = 0.005, maxiter = max_iters, init = None)
    data_copy = pd.DataFrame(data_copy)
    data_copy.columns = data_cols
    data_copy[drop_col] = data_index
    data_copy['Cluster'] = u.argmax(axis = 0)
    return(data_copy)

def generate_MotifeR_input(cluster_dataset, background_dataset, selection_cluster):
    """
    Generates input for MotifeR analysis.
    :param pd.DataFrame() cluster_dataset: Mfuzz output dataset
    :param pd.DataFrame() background_dataset: imputed dataset with group intensities
    :param int selection_cluster: Mfuzz 
    """
    if 'UPD_seq' in background_dataset.columns:
        print('background dataset is valid')
    else:
        print('Sequence information is not available, please use the appropriate background dataset')
    if 'Cluster' in cluster_dataset.columns:
        print('Cluster dataset is valid')
    else:
        print('Cluster assignment is not available, please use Mfuzz first')
    if 'Protein_Collapse_key' in background_dataset.columns:
        output_df = pd.DataFrame()
        out_dict = {}
        tmp_df = pd.DataFrame()
        tmp_df1 = pd.DataFrame()
        for clus in cluster_dataset['Cluster'].unique():
            tmp_df = cluster_dataset[cluster_dataset['Cluster'] == clus].sort_values('PTM_Collapse_key')
            tmp_df = tmp_df.reset_index().drop('index', axis = 1)
            tmp_df1 = background_dataset[background_dataset['Protein_Collapse_key'].isin(tmp_df['PTM_Collapse_key'])].sort_values('Protein_Collapse_key')
            tmp_df1['UPD_seq'] = tmp_df1['UPD_seq'].str.replace('*', '#').str.upper()
            tmp_df1 = tmp_df1.reset_index().drop('index', axis = 1)
            tmp_df['Pepseq'] = tmp_df1['UPD_seq']
            out_dict[clus] = tmp_df
        output_df['Pepseq'] = out_dict[selection_cluster]['Pepseq']
    else:
        output_df = pd.DataFrame()
        out_dict = {}
        tmp_df = pd.DataFrame()
        ...
        for clus in cluster_dataset['Cluster'].unique():
            tmp_df = cluster_dataset[cluster_dataset['Cluster'] == clus].sort_values('PTM_Collapse_key')
            tmp_df = tmp_df.reset_index().drop('index', axis = 1)
            tmp_df1 = background_dataset[background_dataset['PTM_Collapse_key'].isin(tmp_df['PTM_Collapse_key'])].sort_values('PTM_Collapse_key')
            tmp_df1['UPD_seq'] = tmp_df1['UPD_seq'].str.replace('*', '#').str.upper()
            tmp_df1 = tmp_df1.reset_index().drop('index', axis = 1)
            tmp_df['Pepseq'] = tmp_df1['UPD_seq']
            out_dict[clus] = tmp_df
...
def filt_per_percentage(df, min_valid, group = 'group', how = 'all'):
    """
    Extracts ratio of missing/valid values in each column and filters column labels with lower ratio than the minimum threshold defined.

    :param data: pandas dataframe with group as rows and protein identifier as column.
    :param str group: column label containing group identifiers. If None, ratio is calculated across all samples, otherwise is calculated per unique group identifier.
    :param float min_valid: maximum ratio of missing/valid values to be filtered.
    :param str how: define if labels with a higher percentage of missing values than the threshold in any group ('any') or in all groups ('all') should be filtered
    :return: List of column labels below the threshold.

    Example::
        result = extract_percentage_missing(data, min_valid=0.7)
    """
    if group is None:
        tmp_df = df.loc[:, df.notnull().mean() >= min_valid]
    else:
        tmp_df = df.copy()
        tmp_df = tmp_df.set_index(group).notnull().groupby(level = 0).mean()
        if how == 'all':
            tmp_df = tmp_df[tmp_df >=min_valid]
            tmp_df = tmp_df.dropna(axis = 1, how = 'all')
        elif how == 'any':
            tmp_df = tmp_df[tmp_df >=min_valid]
            tmp_df = tmp_df.dropna(axis = 1, how = 'any')
        tmp_df[group] = df[group]
        tmp_df = df[df.columns.intersection(tmp_df.columns)]
    return(tmp_df)
...
def run_ssgsea(data, annotation, annotation_col='annotation', identifier_col='identifier', set_index=[], outdir=None, min_size=15, max_size=500, scale=False, permutations=0):
    """
